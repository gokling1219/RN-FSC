...
def train(im_width, im_height, depth):
    ...
    feature_encoder.load_state_dict(torch.load(str("./model/meta_training_feature_encoder_20way_1shot_newmodel2.pkl"), map_location='cuda:0'))
    print("load feature encoder success")

    relation_network.load_state_dict(torch.load(str("./model/meta_training_relation_network_20way_1shot_newmodel2.pkl"), map_location='cuda:0'))
    ...
    # 训练数据集
    f = h5py.File('data/SA_' + str(im_width) + '_' + str(im_height) + '_' + str(depth) + '_support' + str(n_examples) + '.h5', 'r')
    ...
    torch.save(feature_encoder.state_dict(),str('./model/SA_feature_encoder_20way_1shot_newmodel2_' + str(n_examples) + 'FT_' + str(EPISODE) + '.pkl'))
    torch.save(relation_network.state_dict(),str('./model/SA_relation_network_20way_1shot_newmodel2_' + str(n_examples) + 'FT_' + str(EPISODE) + '.pkl'))
    #torch.save(feature_encoder.state_dict(),str('./model/feature_encoder_20way_1shot_model2_' + str(n_examples) + 'FT_1000.pkl'))
    #torch.save(relation_network.state_dict(),str('./model/relation_network_20way_1shot_model2_' + str(n_examples) + 'FT_1000.pkl'))
